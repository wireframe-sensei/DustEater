...
/// Shows a real `QLThumbnailGenerator` thumbnail for a file, falling back to
/// the given placeholder both while the real thumbnail is loading and for
/// anything QuickLook can't generate one for - never a blank tile. Not
/// limited to images: `QLThumbnailGenerator` handles video, PDF, and many
/// document types too, so this tries every file Explore shows, not just a
/// pre-filtered "is this an image" subset.
struct ThumbnailImageView<Placeholder: View>: View {
    ...
    var body: some View {
        ...
        // Keyed on `path`, not a bare `.task` - this view is reused as list
        // rows/tiles scroll (SwiftUI identity, not literal cell reuse, but
        // the same principle applies), so a bare `.task` would only ever
        // fire once per view *identity*, silently leaving a stale thumbnail
        // showing if the same row instance gets bound to a different path.
        // Keying on `path` reissues (and cancels the previous) request
        // whenever it changes.
        .task(id: path) {
...
final class ThumbnailCache {
    ...
    /// clean content preview of the file itself.
    ///
